...
app.get("/api/health", (req, res) =>
  res.json({
    ...
    security: {
      ...
      rateLimiting: `${limits.max} req / ${limits.windowMs / 1000}s on /api/auth/* (stored in Atlas)`,
    },
  }),
);
...
app.use((err, req, res, _next) => {
  // Client errors: return the right status and don't log them as server failures.
  ...
  // Anything left is a genuine server error.
  console.error(err);
